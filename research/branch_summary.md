...
| Core macro spec fast path | Added conservative fast paths for common valid `clojure.core` macro forms before the spec stack is loaded; invalid or unsupported forms still fall back to the existing spec checker. | Implemented and tested. |
...
| Core macro spec fast path | `teststm.clj` improved from about `1016 ms` median to about `476 ms` median. | Kept. |
