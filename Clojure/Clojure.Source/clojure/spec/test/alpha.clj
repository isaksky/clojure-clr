...
(defn- instrument-choose-fn
  ...
  (if (some #{sym} stub)
    ((requiring-resolve 'clojure.spec.gen.alpha/generate) (s/gen spec over))
    (get replace sym f)))
...
(defn- quick-check
  ...
  (let [{:keys [num-tests] :or {num-tests 1000}} opts
    ...
    (if (throwable? g)
      {:result g}
      (let [for-all* (requiring-resolve 'clojure.spec.gen.alpha/for-all*)
            quick-check (requiring-resolve 'clojure.spec.gen.alpha/quick-check)
            prop (for-all* [g] #(check-call f specs %))]
        (apply quick-check num-tests prop (mapcat identity opts))))))
...
key with a count for each different :type of result."
  ...
  ([check-results summary-result]
     (reduce
      (fn [summary result]
        ((requiring-resolve 'clojure.pprint/pprint) (summary-result result))
        (-> summary
            (update :total inc)
